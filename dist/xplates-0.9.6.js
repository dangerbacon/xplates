...
XPlates = (function()
{
  ...
  //Filter
  function stringify(arg_text, arg_params)
  {
    return JSON.stringify(filter ? filter(arg_text, arg_params) : arg_text);
  }

  //Output verbatim  
  function verbatim(arg_text, arg_params)
  {
    return (arg_text.length ? sIn() + stringify(arg_text, arg_params) : '');
  }
  ...
  {
    //Variable correction
    var funcname = arg_funcname || 'xplate';
    var argnames = arg_argnames || ['arg'];
    ...
    //Wrap it up
    if (index) { jscode += verbatim(arg_code.substr(index, arg_code.length-index), arg_params) + sOut() + 'return ' + retvar + '; }'; }
    else { jscode = "return "+stringify(arg_code, arg_params)+"; }"; varnames = {}; argnames = []; }

    //Build the prefix
    var jsprefix = 'return function ' + funcname + '(' + argnames.join(',') + '){';
    var varcount = 0;
    for (var k in varnames) jsprefix += (varcount++?',':'var ') + k + (typeof varnames[k] === 'undefined' ? '' : '='+JSON.stringify(varnames[k]));
    jsprefix += ';';
